...
<body>
   ...
   <c:when test="${not empty param.id  && not empty param.name && not empty param.price}">       
	...
	<%-- <form action="TicketBookingController" method="post">
		flight_No:<input type="number" name="flight_No" /> 
		Departure_date:<input type="date" name="flight_date" /><br> 
		<label for="flight_class">class:</label>
		<select name="flights_class_name" id="flights_class">
			<option value="First Class">First Class</option>
			<option value=" Business Class">Business Class</option>
			<option value="Economy">Economy</option>
		</select>
		flight_total_no_seats:<c:out value="${pro.flight_No}" />
		<br> no_of_seats:<input type="number" name="no_of_seats" /><br>
